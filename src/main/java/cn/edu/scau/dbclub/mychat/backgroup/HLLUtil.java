...
public class HLLUtil {
    ...
    public static void addLoginUser(String prefix, Integer userId) {
        Date date = new Date();
//        String key = prefix + FORMAT_DAY_MINUTES.format(date);
        String key = prefix + FORMAT_DAY_HOURS.format(date);
        JedisUtil.pfAdd(key, String.valueOf(userId));
    }
...
//    }
    ...
    public static List<String> parse(Date d1, Date d2) {
        ...
        Date dateLastPortionHour = DateUtils.truncate(d2, Calendar.HOUR_OF_DAY);
        long delta2 = d2.getTime() - d1.getTime();
        // -1
        int hoursDiff = (int) (delta2 / DateUtils.MILLIS_PER_HOUR) - 1;
        Date date1Increment = DateUtils.addHours(dateLastPortionHour, -1 * hoursDiff);
        while (dateLastPortionHour.compareTo(date1Increment) >= 0 && hoursDiff >= 0) {
            list.add(FORMAT_DAY_HOURS.format(date1Increment));
            date1Increment = DateUtils.addHours(date1Increment, 1);
...
//        System.out.println(parse(DateUtils.addDays(date, -1), date));
//        System.out.println();
//        System.out.println(parse(DateUtils.addHours(date, -1), date));
//        System.out.println();
//        System.out.println(parse(DateUtils.addMinutes(date, -1), date));


//        FORMAT_DAY_MINUTES.format(date);


        List<String> suffixKeys = HLLUtil.getLastHours(date, 1);
        suffixKeys = HLLUtil.getLastHours(date, 24);
        suffixKeys = HLLUtil.getLastDays(date, 7);
    }
}